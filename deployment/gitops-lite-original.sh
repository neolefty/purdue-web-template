...
# Build and deploy frontend (optional - controlled by GITOPS_BUILD_FRONTEND)
if [ "$BUILD_FRONTEND" = "true" ] && [ -f frontend/package.json ]; then
    ...
    if [ $BUILD_EXIT -eq 0 ]; then
        ...
        log "⚠️ Frontend build skipped (exit code: $BUILD_EXIT, see /tmp/npm-build-$APP_NAME.log)"
    fi
    cd ..
